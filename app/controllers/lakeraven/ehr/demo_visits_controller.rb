...
module Lakeraven
  module EHR
    ...
    class DemoVisitsController < ActionController::Base
      ...
      # RESTful entry: GET patients/:dfn/visits/new renders the same charting form.
      def new
        @patient_name = patient_name(params[:dfn])
        @dfn = params[:dfn]
        @visit = visit_state(@dfn)
      ...
      def close
        ...
        redirect_to patient_visit_summary_path(params[:dfn], DEMO_VISIT_IEN)
      end

      # Clears the in-progress visit so the demo can be run again from scratch.
      def reset
        session[:demo_visits]&.delete(params[:dfn].to_s)
        redirect_to new_patient_visit_path(params[:dfn])
      end

      # Read-only summary of the saved visit — HTML for browsers, FHIR R4 for .json.
      def summary
        @patient_name = patient_name(params[:dfn])
        @dfn = params[:dfn]
        @visit = visit_state(@dfn)
        respond_to do |format|
          ...
          format.json { render json: visit_fhir(@dfn, @visit), content_type: "application/fhir+json" }
        end
      end

      private

      # Minimal FHIR R4 Bundle (Encounter + vitals Observations) for the visit.
      def visit_fhir(dfn, visit)
        entries = [ { resource: {
          resourceType: "Encounter", id: DEMO_VISIT_IEN.to_s,
          status: visit["status"] == "finished" ? "finished" : "in-progress",
          class: { system: "http://terminology.hl7.org/CodeSystem/v3-ActCode", code: "AMB", display: "ambulatory" },
          subject: { reference: "Patient/#{dfn}" },
          reasonCode: (visit.dig("pov", "code") && [ { coding: [ { system: "http://hl7.org/fhir/sid/icd-10-cm", code: visit["pov"]["code"] } ], text: visit.dig("pov", "narrative") } ]) || nil
        }.compact } ]
        (visit["vitals"] || []).each do |m|
          entries << { resource: {
            resourceType: "Observation", status: "final",
            category: [ { coding: [ { system: "http://terminology.hl7.org/CodeSystem/observation-category", code: "vital-signs" } ] } ],
            code: { text: m["label"] || m["abbreviation"] }, subject: { reference: "Patient/#{dfn}" },
            valueQuantity: { value: m["value"], unit: m["units"] }
          } }
        end
