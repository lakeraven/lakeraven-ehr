...
def partner_entries
  partner_bundle["entry"] || []
end

def partner_resources
  partner_entries.map { |e| e["resource"] }
end
...
When("I request the Provenance for the weight by target only") do
  ...
  partner_get("/lakeraven-ehr/Provenance", target: "Observation/#{@wt_obs_id}")
end

When("I request the Provenance resource by its id") do
  partner_get("/lakeraven-ehr/Provenance/prov-#{@wt_obs_id}")
end

When("I read the observation ids for patient {string}") do |dfn|
  partner_get("/lakeraven-ehr/Observation", patient: dfn)
  assert_equal 200, last_response.status
  @first_read_ids = partner_resources.map { |r| r["id"] }
...
When("I read the observation ids for patient {string} again") do |dfn|
  partner_get("/lakeraven-ehr/Observation", patient: dfn)
  assert_equal 200, last_response.status
  @second_read_ids = partner_resources.map { |r| r["id"] }
end

When("I page through {string} for patient {string} with _count {string}") do |path, dfn, count|
  @pages = []
  partner_get(path, patient: dfn, _count: count)
  assert_equal 200, last_response.status
  @pages << partner_bundle
...
Then("every Provenance should be valid FHIR R4") do
  provs = partner_resources.select { |r| r["resourceType"] == "Provenance" }
...
Then("the bundle should contain exactly {int} entry/entries") do |count|
  assert_equal count, partner_entries.length,
    "Expected #{count} entries, got #{partner_entries.length}: #{partner_resources.map { |r| r['id'] }.inspect}"
end

Then("the bundle total should be {int}") do |total|
  assert_equal total, partner_bundle["total"]
end

Then("every Condition in the bundle should have clinical status {string}") do |status|
  conditions = partner_resources.select { |r| r["resourceType"] == "Condition" }
...
Then("the active Condition should be the hypertension problem") do
  condition = partner_resources.find { |r| r["resourceType"] == "Condition" }
...
Then("every MedicationRequest in the bundle should have status {string}") do |status|
  meds = partner_resources.select { |r| r["resourceType"] == "MedicationRequest" }
  assert meds.any?
  meds.each { |m| assert_equal status, m["status"] }
end

Then("every Observation in the bundle should have code {string}") do |code|
  observations = partner_resources.select { |r| r["resourceType"] == "Observation" }
...
Then("the bundle observations should be sorted newest first") do
  times = partner_resources.map { |o| Time.parse(o["effectiveDateTime"]) }
...
Then("every Encounter in the bundle should have a class and a period") do
  encounters = partner_resources.select { |r| r["resourceType"] == "Encounter" }
...
Then("the bundle encounters should be sorted newest first") do
  times = partner_resources.map { |e| Time.parse(e.dig("period", "start")) }
...
def bp_component(code)
  bp = partner_resources.find { |r| r["resourceType"] == "Observation" && r["component"] }
...
Then("the observation ids in the bundle should all be distinct and non-blank") do
  ids = partner_resources.map { |r| r["id"] }
...
Then("exactly {int} Observation in the bundle should have status {string}") do |count, status|
  matching = partner_resources.count { |r| r["resourceType"] == "Observation" && r["status"] == status }
