...
The legacy predecessor monolith bundled clinical workflows (charting, encounters, dashboards, cases, tasks), administrative concerns (account management, billing, subscription), and the Rails plumbing into one app. The decomposition split this into:
...
| Authentication, MFA/2FA, sessions, admin, audit-report UI | `lakeraven-ehr-saas` (host) — host-template concerns; **out of scope for the engine**. See ADR 0001 in that repo. |
