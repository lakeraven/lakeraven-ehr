...
|---|---|---|---|
| 1 | Backend services token obtained with a signed JWT assertion against a published JWKS | see auth PR | `feat/partner-auth-conformance` (worktree `ehr-partner-auth`) |
| 2 | Credential scoped to one organisation; cannot read another's patients | see auth PR | `feat/partner-auth-conformance` |
