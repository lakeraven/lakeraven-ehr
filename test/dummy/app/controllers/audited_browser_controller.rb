...
class AuditedBrowserController < ActionController::Base
  ...
  def show
    ...
    # The two channels a controller cannot reach by clearing response headers:
    # both are committed by MIDDLEWARE after the controller returns. A cookie
    # jar write and a session write must be rolled back at their own layer.
    cookies[:last_patient] = PATIENT_LABEL
    session[:last_patient] = PATIENT_LABEL
